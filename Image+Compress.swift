...
extension UIImage {
    ...
    // MARK: - 降低质量
    public func compress(maxSize: Int =  500) -> Data? {
        
        //先判断当前质量是否满足要求，不满足再进行压缩
        var finallImageData: Data? = self.jpegData(compressionQuality: 1.0);
        
        if let sizeOrigin = finallImageData?.count {
        ...
        let newImage = UIImage.newSizeImage(size: defaultSize, sourceImage: self)
         ...
         */
        //思路：使用二分法搜索
        finallImageData = UIImage.halfFuntion(arr: compressionQualityArr, image: newImage, sourceData: finallImageData!, maxSize: maxSize)
        ...
        while finallImageData?.count == 0 {
            ...
            if let quality = compressionQualityArr.last {
                if let sourceImageData = newImage.jpegData(compressionQuality: quality) {
                    if let compressImage = UIImage.init(data: sourceImageData) {
                        let image = UIImage.newSizeImage(size: defaultSize, sourceImage: compressImage);
                        if let imageData = image.jpegData(compressionQuality: 1.0) {
                            finallImageData = UIImage.halfFuntion(arr: compressionQualityArr, image: image, sourceData: imageData, maxSize: maxSize)
                        }
                    }
                }
            }
...
//            finallImageData = self.halfFuntion(arr: compressionQualityArr.copy() as! [CGFloat], image: image, sourceData: UIImageJPEGRepresentation(image,1.0)!, maxSize: maxSize)
    ...
    private static func newSizeImage(size: CGSize, sourceImage: UIImage) -> UIImage {
    ...
    // MARK: - 二分法
    private static func halfFuntion(arr: [CGFloat], image: UIImage, sourceData finallImageData: Data, maxSize: Int) -> Data? {
